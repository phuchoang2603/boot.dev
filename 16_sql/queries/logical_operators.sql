...
INSERT INTO users(name, age, country_code, username, password, is_admin)
VALUES ('Edge', 18, 'CA', 'edge_case', 'thereIsAlwaysOne', false);


-- Select all users that are from CA and are under 18 years old.
SELECT *
FROM users
where country_code = 'CA'
  AND age < 18;

-- Select all users that are from US or Canada and are under 18 years old.
SELECT *
FROM users
WHERE (country_code = 'US' OR country_code = 'CA')
  AND age < 18;

-- Select all users that are from US, CA, or MX
SELECT *
FROM users
